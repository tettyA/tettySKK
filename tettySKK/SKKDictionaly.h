...
using SKKCandidates = std::deque<SKKCandidate>;//候補文字列群
using SKKDictionary = std::map<std::wstring, SKKCandidates>;//読み，候補文字列群

#define MAX_SKK_DICTIONARY_HISTORY 1000
using SKKdictionaryHistory = std::deque<std::wstring>; //読みのみ 変換履歴
...
public:
	...
	BOOL LoadDictionaryFromFile(const std::wstring& filepath, const std::wstring& userfilepath);
	//BOOL LoadUserDictionaryFromFile(const std::wstring& filepath);
	BOOL SaveDictionaryToUserFile(const std::wstring& filepath) const;

	void GetPredictionCandidate(std::wstring& prefix, SKKCandidate& candidate) const;
	void AddHistoryCandidate(const std::wstring& yomi, const SKKCandidate& candidate);
private:
	SKKDictionary m_dictionary;
	SKKDictionary m_userdictionary;
	SKKdictionaryHistory m_history;
